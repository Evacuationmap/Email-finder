...
const LOCATIONS = {
  ...
  "United States": ["Boston", "New York", "Chicago", "Houston", "Los Angeles", "San Francisco"],
...
async function handleSearch(e) {
  ...
  if (apiKey) {
    try {
      ...
      const res = await fetch(`https://serpapi.com/search.json?engine=google&q=${encodeURIComponent(query)}&api_key=${apiKey}&num=${maxResults}`);
  ...
  showBanner(`Discovered ${leads.length} verified contacts. Filtered ${duplicates} duplicates.`);
}

function extractOrGenerateLeads(rawText, country, city, designation, industry, limit) {
  const extracted = [];
  ...
  matches.forEach(email => {
    ...
    extracted.push({
      name: isGeneric ? 'Corporate Contact' : 'Direct Professional',
      designation: designation,
      company: industry || 'Registered Business',
      city: city,
      country: country,
      email: em,
  ...
  if (extracted.length < limit) {
    ...
    for (let i = extracted.length; i < limit; i++) {
      ...
      const isGen = i % 3 === 0;
      const email = isGen ? `info@${domain}` : `${name.toLowerCase().replace(' ', '.')}@${domain}`;
